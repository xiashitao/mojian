...
## eval 2026-07-17(验证本轮所有改动):均分 4.70(基线 4.38)✅
- 期望全满足;记忆/检索/reflect/稳定性改动无回归。
- 残留两类(均为基线时代旧账,非本轮引入):
  ① guangzhou-2000 忠实性 1 分:模型把食神当用神(引擎判正印)——
    「喜忌极性背叛」的变种:不翻吉凶了,改抢用神认定权。潜在修法:
    _SEC_FACTS 补「用神以引擎为准,不许另行认定」;改 prompt 需再跑 eval。
  ② 干支泄漏:模型原始输出 6 例仍泄(回归信号,按设计打在 raw 上);
    逐一核对全部为**盘内**干支 → 流式净化 100% 覆盖,用户零感知。
    「盘外干支」(模型自算的流年干支)理论上净化器不剔,本轮未出现,留意。

## 用神认定权加固(2026-07-18 已修,eval 基线 4.70→4.78)
- _SEC_FACTS 新增【铁律·用神不可另立】独立段:原禁令("也不得改动用神")
  淹没在 700 字段落中间,模型仍按扶抑另取用神;升格为与「符号不可翻转」
  平级的铁律,并点名绕路(扶抑/调候另取一律作废)。
- guangzhou-2000 加 must_not 回归锚(把非引擎用神当用神组织分析);
  修复验证:忠实性 1→5,总分 3.60→5.00。
- 附带发现:judge 偶发返回非法 JSON 导致「评分失败」,无重试——
  小韧性问题,记待办。

## 运营后台(2026-07-18 已实现)
- 对「不做观测前端」边界的一次有意识放宽,只放运营刚需:
  /admin/ops(反馈列表差评优先 + 最近轮次)→ /admin/conversations/:id
  会话回放(对话式渲染,focus 参数定位被反馈的回复并红框高亮)。
- 每条助手回复带元信息条:反馈标记/意图/话题/耗时/模型调用数/token/¥成本,
  「查看链路」复用 TraceModal。后端 /api/admin/conversations/{id}
  (不做归属过滤,require_admin)+ /api/admin/runs;runs 带 cost
  (JOIN run_costs)。样式遵守无影/素 Rule。
- 用户侧不再展示 analysis_id(admin 保留为会话内 trace 入口)。

## 待办
- 合盘 feature 设计（等 registry 稳定后）
- eval judge 韧性:非法 JSON 无重试,偶发「评分失败」(retries=0?查 judge.py)
- 名人八字:①挑命局结构互补的盘扩 eval 用例(用户把关选盘/时辰考证);
  ②回溯吻合纯引擎测试(史实转折年 vs 引擎流年符号,零 LLM 成本);
  断言不点名(沿 test_known_charts 惯例),只做内部 eval 不进产品。
- 用神认定加固(残留①)+ 下次 eval 一并验证。
