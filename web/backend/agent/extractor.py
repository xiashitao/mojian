...
1. **intent**: 用户意图。必须是以下之一：
   ...
   - "clarify_previous" — 针对上一轮回答的后续反应：追问原因或依据、表达不认同或质疑
     （"你是不是看错了""我觉得不对"）、带情绪的倾诉或求安慰（"我最近很难受，
     告诉我会好起来的""你就不能说点好听的吗"）
   - "collect_birth_info" — 只提供了出生信息，没有明确问题
   - "smalltalk" — 纯打招呼、寒暄、感谢（如"你好""在吗""谢谢"）。注意：只要用户是在
     回应此前的咨询内容——包括情绪化的倾诉、不满、求安慰——就不是 smalltalk，
     应归 "clarify_previous"
